...
async function loadAdminAudit(){adminLoading('admin-audit-list');const{data,error}=await supabaseClient.from('admin_audit_logs').select('id,admin_profile_id,action,entity_type,entity_id,old_values,new_values,reason,created_at').order('created_at',{ascending:false}).limit(500);if(error)return adminFailure('admin-audit-list',error);adminAuditLogs=data||[];const adminIds=[...new Set(adminAuditLogs.map(row=>row.admin_profile_id).filter(Boolean))];if(adminIds.length){const{data:admins}=await supabaseClient.from('users').select('id,name,email').in('id',adminIds);const adminMap=new Map((admins||[]).map(admin=>[admin.id,admin]));adminAuditLogs.forEach(row=>{row.admin_profile=adminMap.get(row.admin_profile_id)||null;});}renderAdminAudit();}
...
function renderAdminAudit(){const q=(document.getElementById('admin-audit-search')?.value||'').toLowerCase(),rows=adminAuditLogs.filter(x=>[x.action,x.entity_type,x.entity_id,x.reason,x.admin_profile?.name,x.admin_profile?.email].some(v=>String(v||'').toLowerCase().includes(q)));document.getElementById('admin-audit-list').innerHTML=rows.map(x=>`<details class="admin-data-card admin-audit-card"><summary><div><strong>${adminEscape(x.action.replaceAll('_',' '))}</strong><span>${adminEscape(x.entity_type)} #${adminEscape(x.entity_id||'—')} · ${adminDate(x.created_at)}</span></div><small>${adminEscape(x.admin_profile?.name||'Admin')}</small></summary><div class="admin-detail-body"><p><strong>Reason:</strong> ${adminEscape(x.reason||'No reason supplied')}</p><div class="admin-audit-changes">${adminAuditChanges(x)||'<span>No field-level change data</span>'}</div></div></details>`).join('')||adminEmpty('No matching audit events');}
