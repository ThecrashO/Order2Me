...
async function loadAdminOrders() {
    ...
    const {data,error}=await supabaseClient.from('orders').select('id,customer_id,customer_name,delivery_note,total_amount,status,created_at,cancellation_reason,shops(name),users!fk_customer(name,email,phone_number),order_items(quantity,price,menu_items(name)),payments(id,payment_method,review_status)').order('created_at',{ascending:false}).limit(300);
    if(error) return adminFailure('admin-order-list',error); adminOrders=data||[]; renderAdminOrders();
}

function renderAdminOrders(){const q=(document.getElementById('admin-order-search')?.value||'').toLowerCase(), status=document.getElementById('admin-order-status')?.value||'all';const rows=adminOrders.filter(o=>(status==='all'||o.status===status)&&[o.id,o.customer_name,o.shops?.name,o.users?.email].some(v=>String(v||'').toLowerCase().includes(q)));const el=document.getElementById('admin-order-list');if(!rows.length){el.innerHTML=adminEmpty('No matching orders');return;}el.innerHTML=rows.map(o=>`<details class="admin-data-card"><summary><div><strong>Order #${o.id}</strong><span>${adminEscape(o.shops?.name||'Unknown shop')} · ${adminEscape(o.customer_name||o.users?.name||'Customer')}</span></div><div><b>${adminMoney(o.total_amount)}</b><span class="admin-status-badge">${adminEscape(o.status)}</span></div></summary><div class="admin-detail-body"><p>${(o.order_items||[]).map(i=>`${adminEscape(i.menu_items?.name||'Item')} ×${i.quantity}`).join(', ')||'No line items'}</p><p><strong>Customer:</strong> ${adminEscape(o.users?.email||'—')} · ${adminEscape(o.users?.phone_number||'—')}</p><p><strong>Note:</strong> ${adminEscape(o.delivery_note||'—')}</p><p><strong>Created:</strong> ${adminDate(o.created_at)}</p>${o.cancellation_reason?`<p class="text-danger"><strong>Cancelled:</strong> ${adminEscape(o.cancellation_reason)}</p>`:''}${!['delivered','cancelled'].includes(o.status)?`<button class="btn btn-danger btn-sm" onclick="cancelAdminOrder(${o.id})">Cancel order</button>`:''}</div></details>`).join('');}
