...
function filterAnalyticsOrders(reset = true) {
    ...
    document.getElementById('analytics-order-table').innerHTML = filtered.length ? `<div class="analytics-table"><div class="analytics-table-head"><span>Order</span><span>Customer</span><span>Items</span><span>Total</span><span>Status</span></div>${visible.map(order => {
        ...
        return `<details class="analytics-order-row"><summary><span class="analytics-order-identity" data-label="Order"><strong>#${order.id}</strong><small>${analyticsDate(order.created_at)}</small></span><span class="analytics-order-customer" data-label="Customer">${analyticsEsc(order.customer_name || 'Unknown')}</span><span class="analytics-order-items" data-label="Items">${items || '—'}</span><span class="analytics-order-total" data-label="Total">${analyticsMoney(order.total_amount)}</span><span class="analytics-order-status order-status order-status--${order.status === 'out_for_delivery' ? 'sent' : order.status === 'delivered' ? 'received' : order.status}">${statusLabels[order.status] || order.status}</span></summary><div class="analytics-order-detail"><div><small>Payment</small><strong>${analyticsEsc(payment?.payment_method || 'Not recorded')}</strong></div><div><small>Delivery note</small><strong>${analyticsEsc(order.delivery_note || 'No note')}</strong></div><div><small>Feedback</small><strong>${getFeedback(order) ? `${getFeedback(order).rating} ★ · ${analyticsEsc(getFeedback(order).comment || 'No comment')}` : 'No feedback'}</strong></div></div></details>`;
